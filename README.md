...
`docker-entrypoint.sh` sources every `*.sh` script in `/docker-entrypoint.d/` in order, then execs the container command. If the command is `init-loop`, it sleeps forever instead (useful for sidecar/init containers).
...
Drop additional `*.sh` scripts into `/docker-entrypoint.d/`. They are sourced in filename order, so use numeric prefixes to control execution order (e.g. `20-custom-setup.sh`).
